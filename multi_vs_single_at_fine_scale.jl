...
function solve_problem_multiscale(A, y; L, ∇L!, Δt, λ, sum_constraint=1,  n_scales=points_to_scale(size(A, 2)),x_init=initialize_x(3; sum_constraint= sum_constraint / scale_to_skip(n_scales)), loss_tol=0.01, max_itr=8000, ignore_warnings=false, show_plot=false, plot_first=7,ground_truth=nothing,plot_init=true,plot_final=true,p=plot(; xlabel="\$t\$", ylabel="\$x\$",legend_columns=2,size=(450,250)),plot_multiples=1,plot_coarse_scales=true)#legendfontsize=12,
    ...
    return x_1, all_iterations, loss_per_itr_all_scales, time_per_scale
end
...
Random.seed!(SEED)
# Run it!!
## First plots are only the first few iterates
plot_multiples = 25
plot_first = 11
show_plot = true
plot_init = true
plot_final = false
plot_coarse_scales = true
time_single = @elapsed xhat, n_itr_single, loss_per_itr_single = solve_problem(A, y; L, ∇L!, loss_tol, Δt, λ, show_plot, plot_first, ground_truth=x, plot_multiples,plot_init,plot_final)
xhat_multi, n_itr_multi, loss_per_itr_multi, time_per_scale = solve_problem_multiscale(A, y; L, ∇L!, loss_tol, Δt, λ, show_plot, plot_first, ground_truth=x,plot_multiples,plot_init,plot_final)
time_multi = sum(time_per_scale)

Random.seed!(SEED) # reset seed so this is an identical run
## Next plots are spaced out apart and includes the final iterate
plot_multiples = 250
plot_first = 17
show_plot = true
plot_init = false
plot_final = true
plot_coarse_scales = false
time_single = @elapsed xhat, n_itr_single, loss_per_itr_single = solve_problem(A, y; L, ∇L!, loss_tol, Δt, λ, show_plot, plot_first, ground_truth=x, plot_multiples,plot_init,plot_final)
xhat_multi, n_itr_multi, loss_per_itr_multi, time_per_scale = solve_problem_multiscale(A, y; L, ∇L!, loss_tol, Δt, λ, show_plot, plot_first=2, ground_truth=x,plot_multiples,plot_init,plot_final,plot_coarse_scales)
time_multi = sum(time_per_scale)

# How to use inkscape to convert from pdf to svg
# inkscape multiscale-ending-iterates.svg --export-type=pdf
...
# Plot scaled loss curves (by time)
# show_plot = false
# time_single = @elapsed xhat, n_itr_single, loss_per_itr_single = solve_problem(A, y; L, ∇L!, loss_tol, Δt, λ, show_plot, plot_first, ground_truth=x, plot_multiples,plot_init,plot_final)
# xhat_multi, n_itr_multi, loss_per_itr_multi, time_per_scale = solve_problem_multiscale(A, y; L, ∇L!, loss_tol, Δt, λ, show_plot, plot_first=2, ground_truth=x,plot_multiples,plot_init,plot_final,plot_coarse_scales)
# time_multi = sum(time_per_scale)
...
# p = plot(; xlabel="time (s)", ylabel="loss")
# n_itr_multi
# times = [0; time_per_scale[begin:end-1]...; ]
